...
router.post("/getItems", async (req, res) => {
  ...
  const emailAccounts = items.filter(
    (item) => item.category == "Email Accounts"
  );
  const sslCertificate = items.filter(
    (item) => item.category == "SSL Certificate"
  );
...
router.post("/submitQuote", async (req, res) => {
  ...
  const emailAccountsMonthlyPrice =
    parseFloat(uPrices.uPrice_emailAccounts) *
    parseFloat(values.value_emailAccounts);
